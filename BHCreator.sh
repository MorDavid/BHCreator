...
echo 'Password: '$(docker logs `docker ps -a | grep _bloodhound | awk "{print $1}"` | grep "Initial Password Set To" --color=never | grep -oP '"message":"#\sInitial\sPassword\sSet\sTo:\s+([^#\s]+)\s+#"' --color=never | awk '{print $6}')
echo ""
